...
public class CosmicCareerProfileStorage implements CareerProfileStorage {
    ...
    public UserProfileSnapshot loadSnapshot(String userId) {
        CosmicDatamodelRecord record = findByUser(CareerLoopDatamodelObjects.PROFILE_SNAPSHOT, userId);
    ...
    public void saveSnapshot(String userId, UserProfileSnapshot snapshot) {
        CosmicDatamodelRecord record = existingOrNew(CareerLoopDatamodelObjects.PROFILE_SNAPSHOT, userId);
    ...
    public Map<String, String> loadFacts(String userId) {
        List<CosmicDatamodelRecord> rows = gateway.list(CareerLoopDatamodelObjects.PROFILE_FACT, userFilter(userId), null);
    ...
    public void saveFact(String userId, String key, String value) {
        CosmicDatamodelRecord record = gateway.findOne(CareerLoopDatamodelObjects.PROFILE_FACT, factFilter(userId, key));
        if (record == null) {
            record = new CosmicDatamodelRecord(CareerLoopDatamodelObjects.PROFILE_FACT)
                    .set(CareerLoopDatamodelObjects.USER_ID, userId)
                    .set("fact_key", key);
        }
        gateway.save(record.set("fact_value", value).set(CareerLoopDatamodelObjects.UPDATED_AT, LocalDateTime.now()));
    }

    public CareerUserProfileDto loadProfile(String userId) {
        CosmicDatamodelRecord record = findByUser(CareerLoopDatamodelObjects.USER_PROFILE, userId);
    ...
    public void saveProfile(String userId, CareerUserProfileDto profile) {
        CosmicDatamodelRecord record = existingOrNew(CareerLoopDatamodelObjects.USER_PROFILE, userId);
    ...
    private CosmicDatamodelRecord existingOrNew(String objectName, String userId) {
        ...
        if (record == null) {
            record = new CosmicDatamodelRecord(objectName).set(CareerLoopDatamodelObjects.USER_ID, userId);
        }
        return record;
    }
