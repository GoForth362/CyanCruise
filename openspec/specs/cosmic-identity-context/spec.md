...
共享服务 SHALL 使用当前 Cosmic 登录上下文决定功能范围。普通用户 SHALL 只能读取和操作其解析后的 `userId` 归属数据；具备管理员等价角色的用户 SHALL 在保留普通用户功能的基础上访问管理后台。
...
- **THEN** 页面 SHALL 清除当前内存中的可用用户身份，并显示需要身份或加载失败状态
...
- **AND** 用户 A 再次登录时 SHALL 能够读取属于用户 A 的分区状态
