...
- webapp 默认 `apiMode` 使用 `server`，仅当 URL 显式设置 `apiMode=direct` 或 `apiMode=kapi` 时切换到其它模式。
- 正式自建应用菜单使用 `apiMode=server`：浏览器通过 KAPI v2 请求已注册的 `/ierp/kapi/v2/v620/v620_cc001/cc001/cyancruise/route`，由现有总路由统一分发 CyanCruise 业务请求；如果后续单独注册 `cc001/cyancruise/server/route`，可通过 `serverApiCode` 参数切换。
- server-managed 代理把当前 Cosmic 登录上下文转换为 `platformIdentity` 传入现有路由，禁止从 KAPI token 前缀、URL 参数或前端 localStorage 推断当前用户。
