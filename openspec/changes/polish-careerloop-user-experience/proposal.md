...
- 将默认首页从“顶部横向导航 + 大块状态摘要”的工程工作台，调整为“金蝶平台侧边栏导航 + CyanCruise 右侧内容页”的功能中心体验。
- 由金蝶平台应用菜单配置“简历”“面试”等侧边栏外部链接；CyanCruise 静态页负责每个外部链接打开后的右侧页面设计。
- 每个外部链接页面展示 IPD 已有核心能力卡片：AI 简历制作、AI 简历修改、全景仿真面试、AI 模拟面试，而不是接口路线或状态面板。
...
- `webapp-careerloop-home-layout`：新增平台菜单外部链接落地页规则，定义每个 hash 页面的内容卡片、页面标题和跳转行为。
