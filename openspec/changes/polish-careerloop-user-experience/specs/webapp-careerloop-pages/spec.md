...
- **THEN** 页面 SHALL 展示 AI简历制作、AI简历修改、简历模板/乔布简历、简历微课等简历相关入口
- **AND** 已接入能力 SHALL 跳转到现有简历或简历诊断 route，未接入能力 SHALL 展示占位状态

#### Scenario: 面试外部链接页展示
- **WHEN** 用户通过金蝶平台菜单打开 `#interview-home`
- **THEN** 页面 SHALL 展示全景仿真面试、AI模拟面试、数字人面试、公务员真题、选调生真题、事业编、大厂真题、面试微课等入口
- **AND** 已接入能力 SHALL 跳转到模拟面试 route 或相关业务页，未接入能力 SHALL 展示占位状态

#### Scenario: 细分外部链接页展示
- **WHEN** 用户打开 `#resume-templates`、`#digital-interview`、`#exam-civil-service` 等细分外部链接
- **THEN** 页面 SHALL 展示该菜单对应的单页内容、能力说明和可用操作
- **AND** 暂未接入真实能力的页面 SHALL 使用禁用卡片或“即将接入”提示，不应展示接口契约
