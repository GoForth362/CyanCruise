...
新增任务输入存储，在 PostgreSQL 中建立 13 张独立功能输入表。表名由固定白名单映射，均包含 `user_id`、`payload_json`、创建时间和更新时间；不同用户的同功能输入以 `user_id` 唯一隔离。分析入口先写入当前功能表，再按当前用户回读该表的 JSON 作为 AI 的 payload。

三个 ApplicationService 在智能体成功返回后创建记录。智能体调用前从对应功能表回读当前用户输入；输出 DTO 和 WebAPI 返回保持不变。
