...
- 将首页“推荐工具”统一为“面试中心”入口，由面试中心再提供“全景仿真面试”和“AI 模拟面试”两种方式，避免首页入口重复。
- 增强苍穹嵌入环境的摄像头兼容：依次尝试当前页面、可访问父页面和旧版媒体接口；仍受浏览器安全策略限制时允许用户选择无摄像头模式继续全景 AI 面试。
- 全景仿真面试在每道题出现后立即开始倒计时，不再等待用户点击“开始回答”；按入门、常规、进阶难度分别提供 3、5、8 分钟，超时后自动提交已有内容或记录本题超时并继续。
- 将 AI 模拟面试记录从答题页和面试中心移到独立记录页面，由后端按当前用户和文字面试类型分页读取，每页固定 10 条；记录继续持久化到既有 Cosmic/PostgreSQL 面试存储。
- 为全景仿真面试增加同级独立记录页面，按 `VOICE/PANORAMA` 类型分页读取，每页固定 10 条，并支持查看结果、查看问答、继续面试和删除记录。
