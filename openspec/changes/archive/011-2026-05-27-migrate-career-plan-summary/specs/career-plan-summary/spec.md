...
系统 SHALL 为每个用户维护最多一份长期求职计划记录。计划记录 SHALL 包含用户 ID、目标岗位、起点状态、阶段里程碑、本周重点、生成来源、生成时间、最后更新时间和版本号。

#### Scenario: 保存新的职业计划
- **WHEN** 用户还没有职业计划且系统保存计划记录
- **THEN** 系统创建该用户的计划，并将版本初始化为 1 或更高的有效版本

#### Scenario: 更新已有职业计划
- **WHEN** 用户已有职业计划且系统保存新的计划内容
- **THEN** 系统更新同一用户的现有计划，并递增版本号，而不是创建第二份当前计划

#### Scenario: 读取当前计划
- **WHEN** 调用方按用户 ID 查询职业计划
- **THEN** 系统返回该用户的当前计划，若不存在则返回明确的空结果
