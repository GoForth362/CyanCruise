...
- [x] 1.2 审阅 `cyancruise-routes.json` 当前 route 列表，标记默认用户可见、debug 可见和 hash 直达可见的页面。
...
- [x] 2.4 更新 `cyancruise-routes.json` 的可见性 metadata，使 route map 能表达默认可见与 debug 可见。
...
- [x] 5.2 运行 `node webapp\isv\v620\cyancruise\validate-routes.js`。
...
- [x] 5.5 同步静态文件到 `F:\kingdee\ENV\static-file-service\isv\v620\cyancruise\` 并本地验证默认模式与 `?ccDebug=1` 模式。

## 6. 平台侧边栏外部链接页面设计

- [x] 6.1 定义金蝶平台应用菜单建议清单，当前按“就业 / 深造”分组；就业只包含 IPD 核心入口：AI简历制作、AI简历修改、全景仿真面试、AI模拟面试；深造仅作为后续规划分组。
...
- [x] 6.10 更新静态资源版本号并同步到 `F:\kingdee\ENV\static-file-service\isv\v620\cyancruise\`。
- [x] 6.11 运行 `node webapp\isv\v620\cyancruise\validate-routes.js`、`node --check webapp\isv\v620\cyancruise\assets\app.js` 和 `openspec validate polish-careerloop-user-experience --strict`。
