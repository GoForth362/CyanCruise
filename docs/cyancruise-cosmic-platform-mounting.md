...
本文档记录 `migrate-cosmic-platform-mounting` 的部署契约。目标是把 `webapp/isv/v620/cyancruise` 中已迁移的 CyanCruise 入口挂载到 Kingdee Cosmic webapp/KDDT/菜单体系，并明确生产身份、开发 fallback、WebAPI 调用和验证边界。

## 资源路径

| 项目 | 值 |
| --- | --- |
| webapp 根目录 | `webapp/isv/v620/cyancruise/` |
| 入口文件 | `index.html` |
| route/API map | `cyancruise-routes.json` |
| 静态脚本 | `assets/app.js` |
| 本地验证脚本 | `validate-routes.js` |
| 平台挂载清单 | `cyancruise-routes.json.platformMounts` |
...
| development | `?identityMode=development&userId=`、`localStorage.cyancruise.userId`、页面输入 | 本地开发、静态验证、接口联调 | SHALL 明确标记为非生产 fallback，不得作为生产身份发布。 |
...
生产后端通过 `CyanCruiseIdentityResolverFactory.production()` 选择 resolver。默认未启用时 SHALL 返回 `IDENTITY_REQUIRED`，不会使用开发 fallback，也不会把请求体 `userId/adminId` 当作生产身份。租户联调确认平台上下文字段后，才通过 system property 启用可配置 adapter。
...
挂载项以 `cyancruise-routes.json.platformMounts` 为准。每个挂载项必须包含：

- `mountKey`：平台挂载唯一键。
- `routeKey`：必须引用 `cyancruise-routes.json.routes[*].key`。
- `title`：菜单或入口标题。
- `target`：如 `index.html#workbench`。
- `audience`：`cyancruise-user` 或 `cyancruise-admin`。
...
建议先发布 `cyancruise-workbench` 作为用户主入口；`cyancruise-admin-console` 只能挂载到管理员菜单或等效 KDDT 管理入口，不得作为普通用户菜单暴露。
...
所有 `/cc001/*` 调用继续以 `cyancruise-routes.json.routes[*].webApis` 为契约来源。用户归属接口必须在解析到用户身份后调用；管理员接口必须在解析到管理员身份和 `ADMIN` 或平台管理员权限后调用。缺失身份时，页面显示 identity-required 或 forbidden 状态，不使用硬编码用户、旧 JWT、旧 admin token 或上一次 localStorage 作为生产身份。

后端通过 `CyanCruiseIdentityResolver` 和 `IdentityAwareCyanCruiseWebApiBoundary` 承接平台身份。生产默认 adapter 在无法读取 Cosmic 登录上下文时返回 `IDENTITY_REQUIRED`，不会信任请求体中的 `userId/adminId` 作为生产身份；开发验证 adapter 必须显式标记为 `development-fallback`。当请求体 `userId/adminId` 与已解析平台身份冲突时，边界返回 `IDENTITY_MISMATCH` 或等价 forbidden 状态，并在调用业务应用服务前终止。
...
| 核查项 | 预期 |
| --- | --- |
| webapp 资源发布 | `index.html`、`assets/*`、`cyancruise-routes.json` 可由 Cosmic webapp 资源访问。 |
