...
| 职业测评 | `AssessmentController`、`AssessmentService`、`AssessmentScale/Question/Option/Record/Answer`、`V5/V8/V10` SQL | 量表启用、题目排序、提交答案、生成结果记录 | `datamodel`、`code/base`、`code/cloud01/v620-cc001-cloud01-app01` | 已完成 DTO、纯 Java 评分内核和画像快照写入，含 MBTI/非 MBTI 画像规则测试与应用服务持久化测试；待 datamodel、WebAPI、页面和 AI 解读适配 | P0 | 评分核心和画像集成已实现：`migrate-assessment-core`、`integrate-assessment-profile-snapshot` |
