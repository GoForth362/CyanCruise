...
| 职业测评 | `AssessmentController`、`AssessmentService`、`AssessmentScale/Question/Option/Record/Answer`、`V5/V8/V10` SQL | 量表启用、题目排序、提交答案、生成结果记录 | `datamodel`、`code/base`、`code/cloud01/v620-cc001-cloud01-app01` | 已完成 DTO 与纯 Java 评分内核，含 MBTI/非 MBTI 画像规则测试；待 datamodel、WebAPI 和画像合并适配 | P0 | 核心规则已实现：`migrate-assessment-core` |
...
| 简历诊断 | `ResumeDiagnosisController`、`ResumeKeywordService`、`PdfTextExtractor` | 简历文本 + JD 输出匹配和建议 | `code/cloud01/v620-cc001-cloud01-app01` | 重写 AI 调用与 PDF 解析适配 | P1 | 待迁移 |
| 职业计划 | `CareerPlanService`、`CareerController`、`UserCareerPlan` | 按目标岗位和用户状态生成计划摘要 | `datamodel`、`code/cloud01/v620-cc001-cloud01-app01` | 在今日任务之后迁入 | P1 | 待迁移 |
| 模拟面试 | `InterviewController`、`InterviewService`、`Interview/InterviewMessage/InterviewQuestion` | 开始面试、对话、结束、报告、历史 | `datamodel`、`code/cloud01/v620-cc001-cloud01-app01`、`webapp` | 拆成题库、会话、报告三个 change | P1 | 待迁移 |
| 助手聊天 | `ChatController`、`AiPersonas`、`AssistantSession/Message` | 多角色提示词共用聊天基础设施 | `datamodel`、`code/cloud01/v620-cc001-cloud01-app01` | 作为 AI 今日任务之后的增强项 | P1 | 待迁移 |
