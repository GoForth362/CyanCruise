...
| 模拟面试 | `InterviewController`、`InterviewService`、`InterviewReportController`、`Interview/InterviewMessage/InterviewQuestion`、`frontend/src/pages/interview` | 开始面试、对话追问、结束、结构化复盘、历史 | `datamodel`、`code/base`、`code/cloud01/v620-cc001-cloud01-app01`、`webapp` | 已完成 JDK 8 会话与报告契约、PostgreSQL 存储、画像同步，并复用简历、目标岗位、用户画像和 provider-neutral AI 网关接通文字开场、逐轮追问、报告缓存、本地降级及中文 webapp 闭环；待题库管理、语音/数字人、身体语言、通知和最终 Cosmic datamodel | P1 | 基础契约：`migrate-interview-core`；文字面试闭环：`enable-ai-mock-interview` |
