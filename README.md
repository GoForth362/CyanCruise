...
CyanCruise 是面向大学生成长与生涯决策的一体化智能平台，以个人画像为数据中枢，将职业测评、简历优化、行动计划、模拟面试、求职辅助与升学规划连接为完整业务链路。

平台不只回答“我适合什么”，更关注“下一步具体做什么”：持续吸收测评结果、简历状态、面试反馈与阶段目标，动态生成可执行任务，并通过 AI 分析与规则引擎帮助用户完成从认识自己到实现目标的全过程。
...
```mermaid
flowchart TB
    UI["CyanCruise Web App<br/>工作台 · 就业 · 升学 · 管理"]
    API["Custom WebAPI<br/>身份校验 · 参数校验 · DTO 契约"]
